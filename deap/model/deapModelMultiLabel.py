...
def read_batches(data_df, batch_size, shuffle ):
    ...
    for i in range(batch_num):
        ...
        # batch_labels = np.array([np.round(row['labels']-1).astype(np.int) for row in batch_data])
...
class CNNClassifier(nn.Module):
    def __init__(self, embed_dim,  in_channels, n_filters, filter_sizes, num_label_arr, dropout_prob, hidden_size):
        ...
        self.linear_arr = nn.ModuleList([nn.Linear(sum(n_filters), hidden_size) for num_labels in num_label_arr])
        self.linear_arr2 = nn.ModuleList([nn.Linear(hidden_size, num_labels) for num_labels in num_label_arr])
        self.dropout = nn.Dropout(dropout_prob)
        # [torch.nn.init.xavier_normal_(cnn.weight) for cnn in self.cnns]
        # [torch.nn.init.xavier_normal_(lin_layer.weight) for lin_layer in self.linear_arr]
        # [torch.nn.init.xavier_normal_(lin_layer.weight) for lin_layer in self.linear_arr2]
    ...
    def forward(self, X):
        ...
        outs = [F.relu(fc(ht)) for fc in self.linear_arr]
        outs2 = [fc(outs[i]) for i,fc in enumerate(self.linear_arr2)]
        return outs2

    def predict(self, X):
        with torch.no_grad():
            outs = self.forward(X)
            # print([out.cpu().numpy() for out in outs])
            predicted_classes = [np.round(out.view(-1).cpu().numpy()) for out in outs]
                #[torch.argmax(out, dim=-1).cpu().numpy() for out in outs]
...
def categorical_accuracy(preds, y):
    ...
    """
    print(list(zip(preds,np.round(y.view(-1).cpu().numpy()))))
    return sum([x == np.round(y) for x,y in zip(preds,y.view(-1).cpu().numpy())])
...
def evaluate(model, eval_dataset, batch_size, criterion):
    ...
    for batch_features, batch_labels in read_batches(eval_dataset, batch_size, False):
        ...
        for i, out in enumerate(outs):
            # print(list(zip(out.view(-1).cpu().numpy(),batch_labels[:, i])))
            losses.append(criterion(
                out,  # (batch_size , num_classes)
                batch_labels[:, i:i+1]  # (batch_size * 1)
            ))
...
def train(train_data, eval_data, test_data, model, batch_size):
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=5, factor=0.33, cooldown=1, min_lr=1e-6)
...
model = CNNClassifier(embed_dim=8064,  in_channels=40, n_filters=[50,100,100,50], filter_sizes=[64,128,512,1024], num_label_arr=1*[1], dropout_prob=0.0, hidden_size=100)
