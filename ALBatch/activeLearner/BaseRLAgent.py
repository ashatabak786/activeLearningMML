...
class BaseAgent(object):
    def __init__(self, config, env, log_dir='./logs/'):
        ...
        self.action_selections = [0 for _ in range(len(env.action_space))]
    ...
    def save_w(self):
        torch.save(self.model.state_dict(), './models/model.dump')
        torch.save(self.optimizer.state_dict(), './models/optim.dump')

    def load_w(self):
        fname_model = "./models/model.dump"
        fname_optim = "./models/optim.dump"
    ...
    def save_replay(self):
        pickle.dump(self.memory, open('./models/exp_replay_agent.dump', 'wb'))

    def load_replay(self):
        fname = './models/exp_replay_agent.dump'
